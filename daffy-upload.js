...
function usage() {
	const usage = commandLineUsage([{header:'daque-upload.js',content:'A CSV/JSON dataset file uploader for the daque server.'},{header: 'Options',optionList: opts_def}]);
	console.log(usage);
	process.exit(0);
}
...
const opts_def = [
	...
	{name:'id',type:String,alias:'i',defaultValue:'?',description:'The dataset ID to use',typeLabel:'<id>'},
...
const uppath = '/upload?'+id+'&'+repeat.toString();
